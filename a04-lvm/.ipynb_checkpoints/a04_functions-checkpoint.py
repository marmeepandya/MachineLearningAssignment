...
def gmm_m(X, W):
    ...
    mu = W.T @ X / normalization.T
    
    X_centered = X[:, None, :] - mu[None, :, :]  # Broadcasting: (N,1,D) - (1,K,D) = (N,K,D)
    
    # compute the outer product
    outer_products = X_centered[:, :, :, None] * X_centered[:, :, None, :]
    
    weighted_outer = W[:, :, None, None] * outer_products
    
    # Sum over data points (axis 0) and normalize
    Sigma = np.sum(weighted_outer, axis=0) / normalization.T[:, None]
